...
struct Module *getModules(pid_t pid, uint32_t *count)
{
        ...
        uintptr_t data = readProcessMemory(pid, dyld_info.all_image_info_addr, &size_infos);
        ...
        uintptr_t info_addr = readProcessMemory(pid, (mach_vm_address_t)infos->infoArray, &size_count);
        ...
        *count = infos->infoArrayCount;
        ...
        for (int i = 0; i < infos->infoArrayCount; i++)
        {
                ...
                uintptr_t fpath_addr = readProcessMemory(pid, (mach_vm_address_t)info[i].imageFilePath, &size_path);
